...
#-------------------------------------------------
epistles_df <- scrape_work(1318, 1)
head(epistles_df)
...
epistles_df$text_raw <- clean_text(epistles_df$text_raw)
head(epistles_df)
#---------------------------------------------------------
...
linha_jesus <- which(rownames(dtm) == "10.96")
...
coords$highlight <- coords$chapter == "10.96"

# ──────────────────────── plot ───────────────────────────
p1 <- ggplot(coords, aes(PC1, PC2)) +
  # pontos “normais”
  ...
  geom_text_repel(data = subset(coords, highlight),
                  ...
                  color = "red", size = 5,
                  box.padding = .4) +
  labs(title    = "Plínio, o Jovem X.96 – PCA das epístolas (feature set Laken)",
       subtitle = "Carta X.96 realçado em vermelho",
       x = "PC1",
       y = "PC2") +
  theme_minimal(base_size = 14)

p2 <- ggplot(coords, aes(PC1, PC2)) +
  # pontos “normais”
  geom_point(data = subset(coords, !highlight),
             color = "grey75", size = 2.5, alpha = .4) +
  # ponto destacado
  ...
  ylim(-7, 7) +
  labs(title    = "Plínio, o Jovem X.96 – PCA das epístolas (feature set Laken)",
       subtitle = "Carta X.96 destacado em vermelho",
       x = "PC1",
       y = "PC2") +
  theme_minimal(base_size = 14)
